...
import { JuypterUser } from '../types';

export const isServerRunning = (user: JuypterUser): boolean => {
...
export const getUTCForActivity = (user: JuypterUser): number => {
  return user.last_activity ? Date.parse(user.last_activity) : 0;
};

export const timeSinceActive = (user: JuypterUser, currentUser:string): string => {
...
export const statusValue = (user: JuypterUser): number => {
  if (user.pending) {
    switch (user.pending) {
      case STARTING_MESSAGE:
...
export const userStatusCompare = (a: JuypterUser, b: JuypterUser): number => {
...
export const userSorter = (column: number, direction: string) => {
  return (a: JuypterUser, b: JuypterUser): number => {
    let cmpValue;
    switch (column) {
      case NAME_SORT:
