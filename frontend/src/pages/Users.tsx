...
import { PRODUCT_NAME, USER_MANAGEMENT_URL } from '../utilities/const';
...
// TODO: Should be moved to network call in a usersService
type UsersResults = {
  users: JuypterUser[];
  loaded: boolean;
  error?: string;
...
const Users: React.FC<UsersPropTypes> = ({ userResults, forceUserUpdate, pageRef, stopping }) => {
  ...
  const startServer = (user: JuypterUser) => {
    window.location.href = `./spawn/${user.name}`;
  };

  const doStopServer = (user: JuypterUser) => {
  ...
  const stopServer = (user: JuypterUser) => {
